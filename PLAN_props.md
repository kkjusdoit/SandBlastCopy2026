...
- **V** 排队一个**天降即炸炸弹**（可瞄准，落地立即炸坑，无 fuse 无拆弹）
...
- **天降炸弹方块**（玩家可控，`ActivePiece.BombKind`）：单格 Mono，从顶部落下可移动/旋转/瞄准；两种落地行为：
  - **变雷（Mine）**：落地锁块时 `LockBombPiece` 写入**同一个地雷材质+fuse**，后续 tick/引爆/拆弹全部复用；下落中渲染成炭黑红脉冲的"已布防地雷"
  - **即炸（Instant）**：落地立即在该格中心 `DetonateBombAt` 炸坑，不留 fuse、不可拆弹；下落中渲染成橙黄警告色。锁块路径已有 `waitingForSandToSettle`，碎沙自动沉降+重扫 bridge，无需额外接线
